use imagelib::{ImageFormat, ImageDecoder, guess_format, ImageError, jpeg, png, gif, webp, tiff,
               bmp, ico, hdr, ppm, FilterType, load_from_memory_with_format};
use std::borrow::Cow;
use std::io::Cursor;
...
fn shrink_inner(data: &[u8], max_dim: u32) -> Result<Cow<[u8]>, ImageError> {
    ...
    let (width, height) = match format {
        ...
        ImageFormat::PPM => decode_dims(ppm::PPMDecoder::new(cursor)?)?,
